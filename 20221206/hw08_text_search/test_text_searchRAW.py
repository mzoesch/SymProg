...
class DocumentCollectionTest(TestCase):
    ...
    def test_cosine(self):
        ...
        self.assertEqual(self.small_collection.cosine_similarity(
            oov_query_doc, collection_doc), 0.)
        # Test search with in-vocabulary query terms
        collection_doc_2 = self.small_collection.docid_to_doc["doc2"]
        self.assertEqual(round(self.small_collection.cosine_similarity(
            inv_query_doc, collection_doc_2), 3), 0.894)
...
class TextDocumentTest(TestCase):
    ...
    def test_preprocessing(self):
        self.assertEqual(
            self.doc1.text, "we are beginning to see some disturbing behavior in certain animals in this area.")
